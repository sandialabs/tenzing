...
/* convert CpuNode to json
*/
void to_json(nlohmann::json& j, const OpBase& n) {
    j = n.json();
}
...
/* convert a shared_ptr<T> to/from json
*/
template <typename T>
void to_json(nlohmann::json& j, const std::shared_ptr<T>& sp) {
    to_json(j, *sp);
}
